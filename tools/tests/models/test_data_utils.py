...
class TestTransformDataNormaliseInputs:
    """transform_data with normalise_inputs=True/False."""

    def test_normalise_inputs_true_fits_normaliser(self):
        """normalise_inputs=True returns a fitted InputNormaliser."""
        x, labels = _make_inputs()
        _, _, normaliser, _ = transform_data(
            x, labels, True, False, torch.float32, torch.device("cpu")
        )
        assert normaliser is not None
        assert normaliser.is_fitted

    def test_normalise_inputs_false_returns_none(self):
        """normalise_inputs=False leaves input_normaliser as None."""
        x, labels = _make_inputs()
        _, _, normaliser, _ = transform_data(
            x, labels, False, False, torch.float32, torch.device("cpu")
        )
        assert normaliser is None

    def test_normalise_inputs_produces_unit_range(self):
        """After normalisation, each feature column spans [0, 1]."""
...
class TestTransformDataStandardiseOutputs:
    ...
    def test_standardise_outputs_true_fits_standardiser(self):
        ...
        _, _, _, standardiser = transform_data(
            x, labels, False, True, torch.float32, torch.device("cpu")
        )
    ...
    def test_standardise_outputs_label_dtype_respected(self):
        ...
        x, labels = _make_inputs()
        _, train_y, _, _ = transform_data(
            x, labels, False, False, torch.float64, torch.device("cpu")
        )
        assert train_y.dtype == torch.float64


class TestTransformDataBothEnabled:
    """transform_data with both normalise_inputs=True and standardise_outputs=True."""
    ...
    def test_both_enabled_returns_fitted_normalisers(self):
        ...
        _, _, normaliser, standardiser = transform_data(
            x, labels, True, True, torch.float32, torch.device("cpu")
        )
    ...
    def test_both_disabled_returns_none_normalisers(self):
        """Neither normaliser nor standardiser is returned when both are False."""
        x, labels = _make_inputs()
        _, _, normaliser, standardiser = transform_data(
            x, labels, False, False, torch.float32, torch.device("cpu")
        )
