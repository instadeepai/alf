...
class TestMLflowLogger:
    ...
    def test_log_calls_mlflow_log_metrics_for_round_metrics(self):
        ...
        with patch("alf_tools.loggers.mlflow_logger.mlflow") as mock_mlflow:
            ml_logger = MLflowLogger("test_experiment")
    ...
    def test_log_round_metrics_uses_round_prefix_and_correct_step(self):
        """Round metrics must be logged with 'round/' prefix and step=round_metrics.round."""
        with patch("alf_tools.loggers.mlflow_logger.mlflow") as mock_mlflow:
            ml_logger = MLflowLogger("test_experiment")
            ...
            # At least one call must contain the round/ prefixed key
            all_keys = [k for c in calls for k in (c.args[0] if c.args else {}).keys()]
            assert any(k.startswith("round/") for k in all_keys)

    def test_log_training_history_uses_per_round_namespace(self):
        """Each round's training history must be logged under round/<N>/training_history/<key>."""
        with patch("alf_tools.loggers.mlflow_logger.mlflow") as mock_mlflow:
            ml_logger = MLflowLogger("test_experiment")
            em = EpochMetrics(epoch=0, train_loss=0.9)
            state = make_state(round_val=2, metrics={}, training_history=[em])
            ml_logger.log(state)
            calls = mock_mlflow.log_metrics.call_args_list
            all_keys = [k for c in calls for k in (c.args[0] if c.args else {}).keys()]
            assert any(k.startswith("round/2/training_history/") for k in all_keys)

    def test_training_history_different_rounds_use_different_namespaces(self):
        """round/0/... and round/1/... must be distinct so data never overwrites."""
        with patch("alf_tools.loggers.mlflow_logger.mlflow") as mock_mlflow:
            ml_logger = MLflowLogger("test_experiment")
            em = EpochMetrics(epoch=0, train_loss=0.9)

            state_r0 = make_state(round_val=0, metrics={}, training_history=[em])
            state_r1 = make_state(round_val=1, metrics={}, training_history=[em])
            ml_logger.log(state_r0)
            ml_logger.log(state_r1)

            calls = mock_mlflow.log_metrics.call_args_list
            all_keys = [k for c in calls for k in (c.args[0] if c.args else {}).keys()]
            assert any(k.startswith("round/0/training_history/") for k in all_keys)
            assert any(k.startswith("round/1/training_history/") for k in all_keys)
