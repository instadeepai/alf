...
from alf_core import (
    BaseModel, Candidate, LabeledCandidates, Predictions,
    Results
)
...
class FeaturizerConfig:
    ...
    custom_featurizer: Callable[
        [list[str]], Float[torch.Tensor, "batch_size n_features"]
    ] | None = None
...
class GPModelTrainer(BaseModel):
    ...
    def train(
        ...
        if val_data is not None and len(val_data) > 0:
            ...
            logger.info(
                f"Validation complete - {val_results.metrics}"
            )
