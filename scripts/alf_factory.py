...
def build_dataset(cfg: DictConfig) -> BaseDataset:
    ...
    Raises:
        ValueError: If cfg.dataset.class_name is not registered.
...
def build_model(cfg: DictConfig) -> BaseModel:
    ...
    Raises:
        ValueError: If cfg.model.class_name is not a known model type.
...
def _build_mlp(mcfg: DictConfig) -> MLPModel:
    ...
    Raises:
        ValueError: If the config contains keys not recognised by MLPModelConfig.
    """
    raw = OmegaConf.to_container(mcfg, resolve=True)
    valid_fields = {f.name for f in dataclasses.fields(MLPModelConfig)}
    structural_keys = {"class_name", "name", "train"}
    unknown = {k for k in raw if k not in valid_fields and k not in structural_keys}
    if unknown:
        raise ValueError(f"Unknown MLPModelConfig field(s): {sorted(unknown)}")
...
def _build_cnn(mcfg: DictConfig) -> CNNModel:
    ...
    Raises:
        ValueError: If the config contains keys not recognised by CNNModelConfig.
    """
    raw = OmegaConf.to_container(mcfg, resolve=True)
    ...
    unknown = {k for k in raw if k not in valid_fields and k not in structural_keys}
    if unknown:
        raise ValueError(f"Unknown CNNModelConfig field(s): {sorted(unknown)}")
...
def build_oracle(cfg: DictConfig, dataset: BaseDataset | None) -> Oracle:
    ...
    Args:
        ...
        dataset: Already-constructed and set-up dataset, used when oracle.mode == 'dataset'.
...
def build_state_loggers(output_dir: Path) -> list[StateLogger]:
