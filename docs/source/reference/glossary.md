...
Definitions of the terms used throughout the ALF documentation. Terms here are
cross-referenceable: other pages link to them with `{term}` roles.
...
Acquisition function
  ...
  Distinct from a {term}`Search function`, which produces the pool the acquisition function scores.

Best-found
  The value of the best candidate discovered so far, tracked as a function of round. The headline
  "are we winning?" curve for a {term}`design <Task>` campaign.

Calibration
  How well a model's stated uncertainty matches its actual error. A well-calibrated
  {term}`Surrogate` "knows what it doesn't know", which is what lets an {term}`Acquisition function`
  explore intelligently. Often summarised by {term}`ECE`.
...
Coverage
  ...
  *the* peak from finding *all* the good designs; related to {term}`Recall`.
...
ECE
  *Expected Calibration Error* — a scalar summary of {term}`Calibration`: the average gap between
...
Modality
  ...
  Determines which {term}`models <Model>` and {term}`datasets <Dataset>` are compatible.
...
Model
  ...
  {term}`Surrogate` or an {term}`Oracle` depending on the role it plays in the loop.

Offline
  A loop in which the {term}`Oracle` reads labels from a held-out, pre-scored pool. Fully
  reproducible with no external calls — the mode used for benchmarking and method development.

Online
  A loop in which the {term}`Oracle` obtains labels from a live scorer (a trained model or
  simulator) on demand — the mode used to drive a real campaign. The loop is otherwise identical to
  {term}`Offline`.

Optimizer
  The object that bundles an {term}`Acquisition function` and a {term}`Search function` and exposes
...
Oracle
  ...
  {term}`Model` (online) or a {term}`Dataset` (offline). Class `Oracle`.

Precision
  Of the candidates a method selected as high-value, the fraction that truly are. Reported
  alongside {term}`Recall` for classification-style objectives.

Recall
  Of all the truly high-value candidates, the fraction the campaign has selected. Measures whether
  the method is finding the good region, not just one point; see also {term}`Coverage`.
...
Search function
  The component that generates the pool of {term}`candidates <Candidate>` an
  {term}`Acquisition function` then scores — e.g. enumerating a dataset (`DatasetSearch`) or
  sampling from a generative {term}`Model` (`GeneratorSearch`). Base class `BaseSearch`.

Spearman correlation
  A rank-correlation metric used to judge how well a {term}`Surrogate`'s predicted ordering of
  candidates matches the true ordering — what matters for selection, regardless of absolute scale.

State
  The mutable carrier threaded through the loop: the current {term}`Dataset`, {term}`Surrogate`,
  round index, acquisition history, and per-round metrics. Class `State`.

Surrogate
  A cheap-to-evaluate probabilistic approximation of the true objective, wrapping a {term}`Model`
  and **retrained each round** on the data acquired so far. Its predictions (and uncertainty) drive
  the {term}`Acquisition function`. Class `Surrogate`.
