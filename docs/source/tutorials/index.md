...
- [MLIP Tutorial](https://github.com/instadeepai/alf/blob/main/tutorials/models/mlip_design_tutorial.ipynb): a machine-learned interatomic potential (MACE force field) as a surrogate over atomistic systems, predicting per-structure energies from ASE `Atoms` objects. The right choice when designing over 3D molecular or crystal structures.
