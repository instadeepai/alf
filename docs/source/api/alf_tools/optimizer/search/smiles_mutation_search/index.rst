...
SMILES edits are structurally invalid, so this filtering step is required. RDKit's error logger
is silenced, since invalid mutations are expected and handled internally.
...
space open at once, so a stall in one neighbourhood doesn't stall the whole search.
