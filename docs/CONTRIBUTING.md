...
Two authoring formats coexist, by design:

- **Narrative pages** (explanation, how-to, tutorial landing pages): write in **MyST Markdown**
  (`.md`) to keep the contribution barrier low.
- **API reference**: stays in reStructuredText (`.rst`), generated by autodoc/autosummary from
  Google-style docstrings. Don't hand-write API pages.
