...
def gfp_dataset():
    ...
    return GFP(
        ...
        seed=51505,
        split_config={
            "split_ratio": {"train": 0.1, "test": 0.2, "validation_frac": 0.5, "candidate_pool": 0.7},
            "split_type": "random"
        }
    )
...
class TestGFPDataset:
    ...
    def test_dataset_split_sizes(self, gfp_dataset):
        ...
        assert len(gfp_dataset.train_dataset) == 50, (
            "Train dataset should have 50 samples"
        )
        assert len(gfp_dataset.test_dataset) == 200, (
            "Test dataset should have 200 samples"
        )
        assert len(gfp_dataset.validation_dataset) == 50, (
            "Validation dataset should have 50 samples"
        )
        assert len(gfp_dataset.candidate_pool) == 700, (
            "Candidate pool should have 700 samples"
        )
    ...
    def test_dataset_data_types(self, gfp_dataset):
        ...
        assert hasattr(gfp_dataset.train_dataset, "labels"), (
            "Train dataset should have labels"
        )
        assert hasattr(gfp_dataset.test_dataset, "labels"), (
            "Test dataset should have labels"
        )
        assert hasattr(gfp_dataset.validation_dataset, "labels"), (
            "Validation dataset should have labels"
        )
    ...
    def test_dataset_no_empty_splits(self, gfp_dataset):
        ...
        assert len(gfp_dataset.validation_dataset) > 0, (
