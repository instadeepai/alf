...
class TestDesignGFPRandomSurrogate:
    ...
    def _assert_dataset_metrics(self, metrics: pd.DataFrame, expected: dict):
        ...
        for metric_name, expected_values in expected.items():
            ...
            assert np.isclose(actual_values, expected_values, atol=1e-10).all(), (
                f"Dataset metric {metric_name} mismatch: expected {expected_values}, got {actual_values}"
            )
    ...
    def _assert_acquired_candidates_metrics(
        ...
        for metric_name, expected_values in expected.items():
            ...
            assert np.isclose(actual_values, expected_values, atol=1e-10).all(), (
                f"Acquired candidates metric {metric_name} mismatch: expected {expected_values}, got {actual_values}"
            )
    ...
    def _assert_surrogate_metrics(self, metrics: pd.DataFrame, expected: dict):
        ...
        for metric_name, expected_values in expected.items():
            ...
            assert np.isclose(actual_values, expected_values, atol=1e-10).all(), (
                f"Surrogate metric {metric_name} mismatch: expected {expected_values}, got {actual_values}"
            )
